...
$("#netWeightIn").val(datas.netWeightIn);
 var hasSettled =datas.hasSettled;
 var notSettled =datas.notSettled;
...
$("#submit").on('click', function () {
    ...
    params.netWeightIn = $("#netWeightIn").val();
    params.notSettled =params.netWeightIn-hasSettled;
...
/*    if ((params.driverName == '' || params.driverName == null) && (params.staffName == '' || params.staffName == null)) {
    ...
    console.log(params);*/
    if (!telTest) {
        var res = Restful.post('bsinventorystorage/update', params);
        if (res.success) {
            alert('保存成功');
