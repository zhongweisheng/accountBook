...
;(function (factory) {
    ...
    $.fn.citys = function(parameter,getApi) {
        ...
        var delay = 200;
        var pfocustime=new Date();
        $('#province').on('focus',function(){
            pfocustime=new Date();
        }).on('blur',function(e){
            if(new Date().getTime()-pfocustime.getTime()<delay){
                $$('#province').focus();
            }
        });

        var cityfocustime=new Date();
        $('#city').on('focus',function(){
            cityfocustime=new Date();
        }).on('blur',function(e){
            if(new Date().getTime()-cityfocustime.getTime()<delay){
                $$('#city').focus();
            }
        });
        var defaults = {
            dataUrl:'accountData/json/citys.json',     //数据库地址
                    ...
                    var format = {
                        ...
                        area:function(){
                            ...
                            if(!hasCity){
                                ...
                                $area.css('display','');
                                if(!options.required){
                                    $area.append('<option value="-1">县</option>');
                                }
                                if(options.nodata=='disabled'){
