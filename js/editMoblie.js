...
var locationText, locationNum, depotId;
var data = Restful.get('bsinventoryrecord/' + id);
var recordType = data.recordType;

//读取粮库
var depotRes = Restful.post('bsinventorydepot/getDepots', {
	userId: userId
});
...
$("#grossWeightIn").val(recordType == 1 ? data.grossWeightIn : data.grossWeightOut);
$("#tareWeightIn").val(recordType == 1 ? data.tareWeightIn : data.tareWeightOut);
$("#netWeightIn").val(recordType == 1 ? data.netWeightIn : data.netWeightOut);
$("#unitPrice").val(data.unitPrice);
$("#moneyIn").val(recordType == 1 ? data.moneyIn : data.moneyOut);
if (data.staffId) {
	$("#salePerson").val(data.staffId).select2();
}
...
/*	    console.log(res);*/
		if (window.confirm("确定要保存到" + testName + "库吗？")) {
			...
			if (res.success) {
				alert('保存成功');
				if (recordType == 1) {
					Restful.operateRecord(id, 1, 12);
					window.location.href = 'inStorage.html';
				} else if (recordType == 2) {
					Restful.operateRecord(id, 2, 22);
