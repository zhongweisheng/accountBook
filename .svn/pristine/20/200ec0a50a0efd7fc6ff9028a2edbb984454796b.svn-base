...
@Repository
public class HtInventoryRecordDao implements
	...
	/***
	 * 修改前台 适用于后台管理 中 ，差别：userId 不是必填项，可以通过用户名查找，返回不同数据。
	 * 
	 * @param isExcel
	 * @param ownerId
	 * @param userId
			...
			if (list != null && list.size() > 0) {
				...
				if (recordType == RecordTypeEnum.入库.getCode()) {
					jsonUtils.addTitle("number", "id", "type", "recordType", "recordNumber", "grainType", "date",
							...
							"staffId", "staffName", "userName");
				} else {
					jsonUtils.addTitle("number", "id", "type", "recordType", "recordNumber", "grainType", "date",
							"time", "licensePlate", "grossWeight", "tareWeight", "deduction", "netWeight", "unitPrice",
							"money", "paidMoney", "unpaidMoney", "ifClear", "quality", "remark", "staffId",
							"staffName", "userName");
				}
					...
					if (recordType == RecordTypeEnum.入库.getCode()) {
						...
						if (isExcel) {
							jsonUtils.addOneRow(number++, item.getId(), searchType, recordName, item.getRecordNumber(),
									...
									remark, item.getStaffId(), item.getStaffName(), userNameTemp);
						} else {
							jsonUtils.addOneRow(number++, item.getId(), searchType, recordName, item.getRecordNumber(),
									grainName, dateReplace(item.getDate()), item.getTime(), item.getLicensePlate(),
									...
									item.getStaffId(), item.getStaffName(), userNameTemp);
						}
					} else {
						// excel 导出 不添加 逗号
						if (isExcel) {
