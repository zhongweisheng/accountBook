...
public class JsonUtils {
	...
	public static String getJV(JsonNode jn, String key, String defaultv) {
		if (jn == null)
			return defaultv;
		JsonNode node = jn.findValue(key);
		if (node != null && !StringUtils.isBlank(node.toString())) {
			return node.asText();
		}
		return defaultv;
	}

	public static Integer getJV(JsonNode jn, String key, Integer defaultv) {
		if (jn == null)
			return defaultv;
		JsonNode node = jn.findValue(key);
		if (node != null) {
