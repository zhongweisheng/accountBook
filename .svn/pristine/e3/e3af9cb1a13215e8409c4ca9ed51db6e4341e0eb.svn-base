...
public class CommonCtrl extends BasicCtrl {
	...
	@ResponseBody
	public Object getImageIds(HttpServletRequest req) {
		int foreignKey = NumberUtils.toInt(req.getParameter("fKey"));
		ImageExample example = new ImageExample();
		example.createCriteria().andForeignkeyEqualTo(foreignKey);
		example.setOrderByClause("id");
		List<Image> list = imageMapper.selectByExample(example);
		Map<String, Object> map = new HashMap<String, Object>();
		if (list != null && list.size() > 0) {
			for (Image image : list) {
				map.put(image.getImagetype(), image.getId());
			}
		}
		return map;
	}
	...
	@ResponseBody
	public Object getImageMap(HttpServletRequest req) {
		int goodsId = NumberUtils.toInt(req.getParameter("goodsId"));
		ImageExample example = new ImageExample();
		example.createCriteria().andForeignkeyEqualTo(goodsId).andImagetypeEqualTo("GOODS_INFO");
