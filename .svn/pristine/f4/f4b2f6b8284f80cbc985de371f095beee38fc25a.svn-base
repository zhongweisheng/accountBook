			...
			if (list != null && list.size() > 0) {
				...
				if (recordType == RecordTypeEnum.入库.getCode()) {
					jsonUtils.addTitle("number", "id", "type", "recordType", "recordNumber", "grainType", "date",
							...
							"staffId", "staffName", "driverName", "depotName");
				} else {
					jsonUtils.addTitle("number", "id", "type", "recordType", "recordNumber", "grainType", "date",
							"time", "licensePlate", "grossWeight", "tareWeight", "netWeight", "unitPrice", "money",
							"deduction", "paidMoney", "unpaidMoney", "ifClear", "quality", "remark", "staffId",
							"staffName", "driverName", "depotName");
				}
				...
				for (BsInventoryTemporary item : list) {
					...
					int depotID = item.getDepotId();
					String depotName = "";
					if (depotID > 0) {
						BsInventoryDepotKey depotKey = new BsInventoryDepotKey();
						depotKey.setId(depotID);

						BsInventoryDepot depotBean = bsInventoryDepotDao.selectByPrimaryKey(depotKey);
						if (depotBean != null) {
							depotName = depotBean.getName();
						}
					}
					if (recordType == RecordTypeEnum.入库.getCode()) {
						...
						if (isExcel) {
							jsonUtils.addOneRow(number++, item.getId(), searchType, recordName, item.getRecordNumber(),
									...
									formatMoneyPercent(item.getGrossWeightIn()), formatMoneyPercent(item
											.getTareWeightIn()), formatMoneyPercent(item.getNetWeightIn()),
									...
									formatMoneyPercent(item.getUnpaidMoney()), item.getIfClear(),
									formatToPercentP3(item.getAveragePriceIn() == null ? 0
											: item.getAveragePriceIn() / 2), quality, remark, item.getStaffId(), item
											.getStaffName(), item.getDriverName(), depotName);
						} else {
							jsonUtils.addOneRow(number++, item.getId(), searchType, recordName, item.getRecordNumber(),
									grainName, dateReplace(item.getDate()), item.getTime(), item.getLicensePlate(),
									formatMoneyPercentCMA(item.getGrossWeightIn()), formatMoneyPercentCMA(item
											.getTareWeightIn()), formatMoneyPercentCMA(item.getNetWeightIn()),
									formatToPercentP3CMA(item.getUnitPrice()),
									formatMoneyPercentCMA(item.getMoneyIn()),
									formatToPercentP3CMA(item.getDeduction()), formatMoneyPercentCMA(item
											.getPaidMoney()), formatMoneyPercentCMA(item.getUnpaidMoney()), item
											.getIfClear(), formatToPercentP3CMA(item.getAveragePriceIn() == null ? 0
											: item.getAveragePriceIn() / 2), quality, remark, item.getStaffId(), item
											.getStaffName(), item.getDriverName(), depotName);
						}
					} else {
						...
						if (isExcel) {
							...
							jsonUtils.addOneRow(number++, item.getId(), searchType, recordName, item.getRecordNumber(),
									...
									item.getStaffId(), item.getStaffName(), item.getDriverName(), depotName);
						}
					}
				}
			}
