...
public class CommonSqlProvider {
	...
	public String selectMlbUserList(Map<String, Object> params) {
		int industry = (int) params.get("industry");
		int foodType = (int) params.get("foodType");
		...
		query.append("  aa.lng, ");
		query.append("  aa.Lat, ");
		query.append("  aa.Name ");
		query.append("FROM mlbuser mu,mlbuser_addition ma,  ");
		query.append("  attr_area aa ,");
		query.append("  bs_user_level  ul,");
		query.append("  bs_temp_trade tt ");
		query.append("WHERE mu.location = aa.ID ");
		query.append(" and   mu.phone = ul.user_phone  ");
		query.append(" and  mu.userId = ma.user_id ");
		query.append(" and  mu.userId = tt.buy_id ");
		if (foodType > 0) {
			query.append(" AND tt.buy_food_type = " + foodType);
		}
		if (StringUtils.isNotEmpty(location)) {
			query.append(" AND mu.location LIKE '" + location + "%' ");
		}
		query.append("  AND ma.industry =  " + industry);
		query.append(" ORDER BY ul.user_level DESC  ");
		// industry =2 ， 深加工厂
		return query.toString();
	}

	public String selectMlbUserHasBuyList(Map<String, Object> params) {
		// int industry = (int) params.get("industry");
		int foodType = (int) params.get("foodType");
		String location = (String) params.get("location");
		...
		query.append("  attr_area aa ,");
		// query.append("  bs_user_level  ul,");
		query.append("  bs_user_require tt ");
		query.append("WHERE mu.location = aa.ID ");
		// query.append(" and   mu.phone = ul.user_phone  ");
		query.append(" and  mu.userId = ma.user_id ");
		query.append(" and  mu.userId = tt.buy_id ");
		if (StringUtils.isNotEmpty(createTime)) {
			query.append(" AND tt.create_time > '" + createTime + "' ");
		}
		if (foodType > 0) {
			query.append(" AND tt.buy_food_type = " + foodType);
		}
		...
		// query.append("  AND ma.industry =  " + industry);
		query.append(" ORDER BY tt.create_time DESC  ");
		// industry =2 ， 深加工厂
		return query.toString();
	}
	...
	public String getSearchCount(Map<String, Object> params) {
		...
		if (StringUtils.isNotEmpty(startDate) && StringUtils.isNotEmpty(endDate)) {
			query.append("    AND t.record_time >= '" + startDate + "' ");
			query.append("    AND t.record_time <= '" + endDate + "' ");
		}
	...
	public String getSearchStorageCount(Map<String, Object> params) {
		String depotIds = (String) params.get("depotIds");
		int userId = (int) params.get("userId");
		int grainType = (int) params.get("grainType");
		int recordType = (int) params.get("recordType");
		...
		query.append("SELECT");
		query.append(" count(1) ");
		query.append("FROM `bs_inventory_storage` t ");
		query.append("WHERE t.user_id = " + userId + " ");
		if (recordType > 0) {
			query.append("    AND t.record_type = " + recordType + " ");
		}
	...
	public String getSearchStorageList(Map<String, Object> params) {
		String depotIds = (String) params.get("depotIds");
		int userId = (int) params.get("userId");
		int grainType = (int) params.get("grainType");
		int recordType = (int) params.get("recordType");
		...
		query.append("  SUM(`net_weight_out`)    netOutTotal, ");
		query.append("  SUM(`money_out`)    moneyOutTotal, ");
		query.append("  SUM(t.paid_money) paidMoney, ");
		query.append("  SUM(IF(t.if_clear=1 ,0, t.unpaid_money)) unpaidMoney, ");
		query.append("  SUM(t.has_settled) hasSettled, ");
		query.append("  SUM(t.not_settled) notSettled, ");
		query.append("  SUM(t.deduction) deduction, ");
		query.append("  SUM(`money_in`)/SUM(`net_weight_in`)/2  averageCost, ");
		query.append("  SUM(`money_out`)/SUM(`net_weight_out`)/2  averagePrice ");
		query.append("FROM `bs_inventory_storage` t ");
	...
	public String findRmkcqk(Map<String, Object> param) {
		...
		/*
		 * query.append(
		 * " SELECT balance,grain_type,average_price_in/2  average_price_in ");
		 * query.append(" FROM (SELECT * ");
		 * query.append(" FROM bs_inventory_record ");
		 * query.append(" where 1=1 "); if (userId > 0) {
		 * query.append(" and  user_id = " + userId); }
		 * 
		 * if (datetime != null && !"".equals(datetime)) {
		...
		query.append(" SELECT grain_type ,IFNULL(SUM(net_weight_in),0) -IFNULL(SUM(net_weight_out),0) balance , FORMAT(IFNULL(SUM(money_in),0)/SUM(net_weight_in)/2,4) average_price_in");
		query.append(" FROM bs_inventory_record   ");
		query.append(" WHERE 1 = 1  ");
		if (userId > 0) {
			query.append(" and  user_id = " + userId);
		...
		if (datetime != null && !"".equals(datetime)) {
			query.append(" AND record_time <= '" + datetime + " 23:59:59' ");
		}
	...
	public String getInventoryStorgeStat(Map<String, Object> params) {
		String depotIds = (String) params.get("depotIds");
		int userId = (int) params.get("userId");
		String datetime = params.get("dateTime").toString();
		StringBuffer query = new StringBuffer(100);
		query.append(" SELECT user_id , ");
		query.append(" 	SUM(`paid_money`) paidMoney, ");
		query.append(" 	SUM(`unpaid_money`) unpaidMoney, ");
		query.append(" 	SUM(`has_settled`) hasSettled,  ");
		query.append(" 	SUM(`not_settled`) notSettled ");
		query.append(" FROM bs_inventory_storage ");
		query.append(" where 1=1 ");
		if (userId > 0) {
			query.append(" and  user_id = " + userId);
		}
	...
	public String getUserClickCount(Map<String, Object> params) {
		int userId = (int) params.get("userId");

		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append("  target_id    userId, ");
		query.append("  SUM(IF(operate_type=1,1,0))    viewCount, ");
		query.append("  (SELECT COUNT(DISTINCT ");
		query.append("     (subject_user_id)) ");
		query.append("   FROM bs_attention ba ");
		query.append("   WHERE ba.object_user_id = t1.target_id and ba.status !=3)    attentionCount, ");
		query.append("  SUM(IF(operate_type=3,1,0))    phoneCount, ");
		query.append("  0            tradeCount ");
		query.append("FROM bs_click_count t1 ");
		query.append("WHERE t1.target_type = 2 ");
		// query.append("    AND t1.target_id = 100 ");
		if (userId > 0) {
			query.append("    AND t1.target_id = " + userId);
		}
		return query.toString();
	}
	...
	public String getUserAuth(Map<String, Object> params) {
		...
		query.append("SELECT ");
		query.append("  count(1) ");
		query.append("FROM bs_user_hzk  ");
		query.append("WHERE 1 = 1 ");
		query.append(" and user_phone = '" + phone + "'");

		return query.toString();
	}

	public String getUserHzk(Map<String, Object> params) {
		String phone = (String) params.get("phone");
		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append("  count(1) ");
	...
	public String getUserRz(Map<String, Object> params) {
		String phone = (String) params.get("phone");
		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append("  count(1) ");
		query.append("FROM bs_user_rz  ");
		query.append("WHERE 1 = 1 ");
		query.append(" and user_phone = '" + phone + "'");
		return query.toString();
	}

	public String getUserLzb(Map<String, Object> params) {
		String phone = (String) params.get("phone");
		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append("  count(1) ");
		query.append("FROM bs_inventory_user  ");
		query.append("WHERE 1 = 1 ");
		query.append(" and phone = '" + phone + "'");
		return query.toString();
	}

	public String getUserLevel(Map<String, Object> params) {
		String phone = (String) params.get("phone");
		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append("  IFNULL(SUM(user_level),0)  userLevel ");
		query.append("FROM bs_user_level  ");
		query.append("WHERE 1 = 1 ");
		query.append(" and user_phone = '" + phone + "'");
		return query.toString();
	}

	public String getUserComment(Map<String, Object> params) {
		int userId = (int) params.get("userId");
		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append("  IFNULL(AVG(comment_level),0)  commentLevel ");
		query.append("FROM bs_user_comment  ");
		query.append("WHERE 1 = 1 ");
		query.append(" and user_id = '" + userId + "'");
		return query.toString();
	}

	public String getUserLevelList(Map<String, Object> params) {
		int userId = (int) params.get("userId");
		StringBuffer query = new StringBuffer(100);
		query.append("SELECT ");
		query.append(" * ");
